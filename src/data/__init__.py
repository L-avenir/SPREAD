...
from .CAST_point_proc_dataset import Cached_CAST_Point_Proc_Dataset
...
def get_raw_dataset(
    ...
    if "cached" in dataset_type:
        if 'point' in dataset_type:
            print("Using cached CAST Point Proc Dataset")
            dataset = Cached_CAST_Point_Proc_Dataset(
...
def get_encoded_dataset(
    ...
    _, encoding = get_dataset_raw_and_encoded(
        config, filter_fn, path_to_bounds, augmentations, split, Uni3D_encoder
    )
    return encoding
