...
def compute_loc_rel(corners1: ndarray, corners2: ndarray, name1: str, name2: str) -> Optional[str]:
    ...
    box1 = corners1[[0, 1, 4, 5], :][:, [0, 2]]
    box2 = corners2[[0, 1, 4, 5], :][:, [0, 2]]

    polygon1, polygon2 = Polygon(box1[[0, 1, 3, 2], :]), Polygon(box2[[0, 1, 3, 2], :])
...
def fill_templates(
    ...
    obj_class_ids = desc["obj_class_ids"]

    selected_relation_indices = np.random.choice(
        len(desc["obj_relations"]),
        min(np.random.choice([1, 2]), len(desc["obj_relations"])),
        replace=False
    )
    ...
    if return_obj_ids:
        ...
        return text, selected_relations, selected_descs
