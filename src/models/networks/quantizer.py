...
class GumbelQuantize(nn.Module):
    ...
    def forward(self, z: Tensor, temperature: Optional[float]=None, kl_weight: Optional[float]=None):
        ...
        kl_weight = self.kl_weight if kl_weight is None else kl_weight
        ...
        z_q = soft_one_hot @ self.embed.weight
    ...
    def get_codebook_entry(self, indices: LongTensor):
        ...
        one_hot: Tensor = F.one_hot(indices, num_classes=self.n_embed).float()
        z_q = one_hot @ self.embed.weight
        z_q = z_q.view(*shape, -1)
        return z_q
